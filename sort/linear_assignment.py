...
def min_cost_matching(distance_metric, max_distance, tracks, detections, track_indices=None, detection_indices=None):
    ...
    # Checking for unmatched detections
    for col, detection_idx in enumerate(detection_indices):
        if col not in indices[:, 1]:
            unmatched_detections.append(detection_idx)

    # Checking for unmatched trackers
...
def gate_cost_matrix(kf, cost_matrix, tracks, detections, track_indices, detection_indices, gated_cost=float('inf'), only_position=False):
    '''
    Invalidates the infeasible entries in cost matrix based on the state distributions obtained by Kalman Filtering.
    '''
